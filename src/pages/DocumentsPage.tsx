...
export function DocumentsPage() {
  ...
  return (
    <div>
      <PageHeader title="Documentos" subtitle="Private evidence storage linked to supplier intelligence" action={suppliers && suppliers.length > 0 ? <Button onClick={openCreate}><Plus size={16} /> Add Document</Button> : undefined} />
      {visibleDocs.length === 0 ? <Card><EmptyState icon={<FileText size={28} />} title="No documents registered" message={suppliers && suppliers.length > 0 ? 'Upload COA, SGS, TDS, SDS/FDS, ISCC, licenses, bills of lading and inspection reports.' : 'Register a supplier first, then add associated documents.'} action={suppliers && suppliers.length > 0 ? <Button onClick={openCreate}><Plus size={16} /> Add Document</Button> : undefined} /></Card> :
      ...
      <Modal open={showForm} onClose={() => setShowForm(false)} title="Agregar documento" footer={<><Button variant="secondary" onClick={() => setShowForm(false)}>Cancelar</Button><Button onClick={save} disabled={!form?.title || !form?.supplier_id}><Upload size={15} /> Upload</Button></>}>
        {form && <div className="space-y-3"><Select label="Proveedor" value={form.supplier_id} onChange={(v) => setForm({ ...form, supplier_id: v })} options={(suppliers ?? []).map((s) => ({ value: s.id, label: s.legal_name || s.trading_name || 'Sin nombre' }))} required /><Input label="Título del documento" required value={form.title} onChange={(v) => setForm({ ...form, title: v })} /><Select label="Tipo de documento" value={form.doc_type} onChange={(v) => setForm({ ...form, doc_type: v as DocumentType })} options={DOC_TYPE_OPTIONS} /><TextArea label="Description" value={form.description} onChange={(v) => setForm({ ...form, description: v })} /><input type="file" className="block w-full text-sm" onChange={(e) => setFile(e.target.files?.[0] ?? null)} /><Input label="Cargado por" value={form.uploaded_by} onChange={(v) => setForm({ ...form, uploaded_by: v })} /><Select label="Verification Status" value={form.verification_status} onChange={(v) => setForm({ ...form, verification_status: v as VerificationStatus })} options={VERIFICATION_OPTIONS} /><p className="text-xs text-gray-400">Los archivos se almacenan en el bucket de documentos de Supabase. La vista previa aparece dentro del perfil del proveedor.</p></div>}
      </Modal>
    </div>
  );
}