...
import { getProductooFamilia, PRODUCT_FAMILY_LABELS, type ProductooFamilia } from '@/utils/productFamilies';
import { Plus, Package, Trash2 } from 'lucide-react';
import type { Productoo, FeedstockType, VerificaciónStatus } from '@/types';
...
function emptyForm(supplierId: string): Omit<Productoo, 'id' | 'created_at' | 'updated_at'> {
...
export function ProductoosPage() {
  const { procurementDomain } = useNav();
  const [showForm, setShowForm] = useState(false);
  const [form, setForm] = useState<Omit<Productoo, 'id' | 'created_at' | 'updated_at'> | null>(null);
  ...
  const families = useMemo(() => {
    const map = new Map<ProductooFamilia, Productoo[]>();
    for (const product of products ?? []) { const family = getProductooFamilia(product); map.set(family, [...(map.get(family) ?? []), product]); }
  ...
  return (
    <div>
      <PageHeader title="Productooos" subtitle="Cinco familias de procurement; las variantes específicas permanecen dentro de cada familia." action={suppliers && suppliers.length > 0 ? <Button onClick={openCrear}><Plus size={16} /> Agregar producto</Button> : undefined} />
      ...
      <Modal open={showForm} onClose={() => setShowForm(false)} title="Agregar producto" footer={<><Button variant="secondary" onClick={() => setShowForm(false)}>Cancelar</Button><Button onClick={save} disabled={!form?.name || !form?.supplier_id}>Crear</Button></>}>
        {form && <div className="space-y-3"><Seleccionar label="Proveedor" value={form.supplier_id} onChange={(v) => setForm({ ...form, supplier_id: v })} options={(suppliers ?? []).map((s) => ({ value: s.id, label: s.legal_name || s.trading_name || 'Sin nombre' }))} required /><Input label="Nombre del producto / variante" required value={form.name} onChange={(v) => setForm({ ...form, name: v })} /><Seleccionar label="Tipo de feedstock" value={form.feedstock_type} onChange={(v) => setForm({ ...form, feedstock_type: v as FeedstockType })} options={FEEDSTOCK_OPTIONS} /><Input label="Origen" value={form.origin} onChange={(v) => setForm({ ...form, origin: v })} /><Input label="Composición" value={form.composition} onChange={(v) => setForm({ ...form, composition: v })} /><div className="grid grid-cols-2 gap-3"><Input label="Volumennn disponible" value={form.available_volume} onChange={(v) => setForm({ ...form, available_volume: v })} /><Input label="Unidad" value={form.unit} onChange={(v) => setForm({ ...form, unit: v })} /></div><Seleccionar label="Estado de verificación" value={form.verification_status} onChange={(v) => setForm({ ...form, verification_status: v as VerificaciónStatus })} options={VERIFICATION_OPTIONS} /></div>}
      </Modal>
    </div>
  );
}