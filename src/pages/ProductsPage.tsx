...
export function ProductsPage() {
  ...
  return (
    <div>
      <PageHeader title="Productos" subtitle="Cinco familias de procurement; las variantes específicas permanecen dentro de cada familia." action={suppliers && suppliers.length > 0 ? <Button onClick={openCrear}><Plus size={16} /> Agregar producto</Button> : undefined} />
        ...
        <div className="space-y-4">
          {families.map(([family, familyProducts]) => (
            <Card key={family}><CardBody>
              <div className="flex items-center justify-between gap-4 mb-3"><div><h3 className="text-sm font-semibold text-gray-900">{PRODUCT_FAMILY_LABELS[family]}</h3><p className="text-xs text-gray-500">{familyProducts.length} registro(s) vinculados a proveedores.</p></div><Badge color="blue">{familyProducts.length} registros</Badge></div>
