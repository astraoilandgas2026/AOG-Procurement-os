import type { AppData, ProcurementDomain, Supplier, Contact, Product, TechnicalSpec, CommercialOffer, Certification, DocumentRecord, DueDiligenceItem, LogisticsInfo, TimelineEvent, FollowUp, RedFlag, IntelligenceFact } from '@/types';

export interface SupplierRepository { getAll(): Promise<Supplier[]>; getById(id:string):Promise<Supplier|null>; getByDomain(domainId:string):Promise<Supplier[]>; getByDomainKey(domainKey:ProcurementDomain):Promise<Supplier[]>; createForDomain(domainKey:ProcurementDomain, data:Omit<Supplier,'id'|'created_at'|'updated_at'>):Promise<Supplier>; create(data:Omit<Supplier,'id'|'created_at'|'updated_at'>):Promise<Supplier>; update(id:string,data:Partial<Supplier>):Promise<Supplier>; remove(id:string):Promise<void>; }
export interface ContactRepository { getAll():Promise<Contact[]>; getByDomainKey(domainKey:ProcurementDomain):Promise<Contact[]>; getBySupplier(supplierId:string):Promise<Contact[]>; create(data:Omit<Contact,'id'|'created_at'>):Promise<Contact>; update(id:string,data:Partial<Contact>):Promise<Contact>; remove(id:string):Promise<void>; }
export interface ProductRepository { getAll():Promise<Product[]>; getByDomainKey(domainKey:ProcurementDomain):Promise<Product[]>; getBySupplier(supplierId:string):Promise<Product[]>; getById(id:string):Promise<Product|null>; create(data:Omit<Product,'id'|'created_at'|'updated_at'>):Promise<Product>; update(id:string,data:Partial<Product>):Promise<Product>; remove(id:string):Promise<void>; }
...
export interface IntelligenceFactRepository { getByEntity(entityType:string, entityId:string):Promise<IntelligenceFact[]>; }
