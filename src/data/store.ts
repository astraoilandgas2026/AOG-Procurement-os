...
export interface SupplierRepository { getAll(): Promise<Supplier[]>; getById(id:string):Promise<Supplier|null>; getByDomain(domainId:string):Promise<Supplier[]>; getByDomainKey(domainKey:'feedstock'|'energy_commodities'):Promise<Supplier[]>; create(data:Omit<Supplier,'id'|'created_at'|'updated_at'>):Promise<Supplier>; update(id:string,data:Partial<Supplier>):Promise<Supplier>; remove(id:string):Promise<void>; }
export interface ContactRepository { getAll():Promise<Contact[]>; getBySupplier(supplierId:string):Promise<Contact[]>; create(data:Omit<Contact,'id'|'created_at'>):Promise<Contact>; update(id:string,data:Partial<Contact>):Promise<Contact>; remove(id:string):Promise<void>; }
export interface ProductRepository { getAll():Promise<Product[]>; getBySupplier(supplierId:string):Promise<Product[]>; getById(id:string):Promise<Product|null>; create(data:Omit<Product,'id'|'created_at'|'updated_at'>):Promise<Product>; update(id:string,data:Partial<Product>):Promise<Product>; remove(id:string):Promise<void>; }
